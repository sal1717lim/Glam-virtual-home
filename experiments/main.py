...
from utils.generate_prompt import obs2text,obs_to_glam_obs,generate_prompt
...
def make_env(env_id, seed, idx, capture_video, run_name, env_params):
    def thunk():
        ...
        #env = gym.wrappepiprs.RecordEpisodeStatistics(env)
        if capture_video:
            if idx == 0:
...
def main(config_args):
    ...
    run_name = f"{name_env}__watch_tv_ppo_llm__{seed}__{time_str}"
    env_params = {
                    'seed': 100,
                    'debug': False,
                }
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(name_env, 100 + i, i, False, run_name, env_params) for i in range(config_args.rl_script_args.number_envs)]
    )
    # Create LLM agent
    
    next_obs =envs.reset()
    
    infos=[obs_to_glam_obs(o) for o in next_obs]
    ep_ret, ep_len = [0 for _ in range(config_args.rl_script_args.number_envs)], \
    ...
    for epoch in tqdm(range(config_args.rl_script_args.epochs)):
        ...
        for t in tqdm(range(config_args.rl_script_args.steps_per_epoch // config_args.rl_script_args.number_envs),
                      ascii=" " * 9 + ">", ncols=100):
            infos=[obs_to_glam_obs(o) for o in next_obs]
            obs=[p["descriptions"] for p in infos]
            possible_actions=[p["possible_actions"] for p in infos]
            prompts=[generate_prompt(_buff, _o, _i) for _buff, _o, _i in zip(transitions_buffer, obs, infos)]
